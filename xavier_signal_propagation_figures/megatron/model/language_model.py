    ...
    def __init__(self,
        ...
        if self.pre_process:
            if args.xavier_flag_sigma:
                xavier_flag_sigma_embedding = math.sqrt(1/(args.hidden_size)) #0.548
                # xavier_flag_sigma_embedding = math.sqrt((1-0.1)/(3)) #0.548
                # xavier_flag_sigma_embedding = 1 #0.548
            else:
                xavier_flag_sigma_embedding = 0.02
            self.embedding = Embedding(self.hidden_size,
                                       args.padded_vocab_size,
                                       args.max_position_embeddings,
                                       args.hidden_dropout,
                                       self.init_method(xavier_flag_sigma_embedding),
                                       self.num_tokentypes)
        ...
        if self.post_process:
            ...
            if self.add_pooler:
                if args.xavier_flag_sigma:
                    xavier_flag_sigma_pooler = 1/math.sqrt(args.hidden_size) #0.044
                else:
                    xavier_flag_sigma_pooler = 0.02
                self.pooler = Pooler(self.hidden_size, self.init_method(xavier_flag_sigma_pooler))
