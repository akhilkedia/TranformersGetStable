...
def simulate_one(args):
    ...
    layer_output = torch.nn.functional.relu(input_tensor)
    ...
    pred_x_out_mean = sigma_x_inp / np.sqrt(2 * np.pi)
    pred_x_out_var = sigma_x_inp * sigma_x_inp * (np.pi - 1) / (2 * np.pi)
    pred_x_out_cov = sigma_x_inp * sigma_x_inp * (np.pi * corr_x_inp + np.sqrt(1 - corr_x_inp**2) - corr_x_inp*np.arccos(corr_x_inp) - 1) / (2 * np.pi)
    # pred_g_out_mean = 0
    pred_g_out_var = 0.5 * sigma_g_inp * sigma_g_inp
    pred_g_out_cov = (0.25+np.arcsin(corr_x_inp)/(2*np.pi)) * sigma_g_inp * sigma_g_inp * corr_g_inp
