...
MEAN_INP = np.linspace(-10, 10, num=3, endpoint=True)
...
DROP_PROB = np.linspace(0.0, 1, num=3, endpoint=False)
...
varying_vals = [MEAN_INP, SIGMA_X_INPUT, CORR_X_INPUT, SIGMA_G_INPUT, CORR_G_INPUT, DROP_PROB, DIM1, SEQ_LEN, [1], RUNS]
...
def simulate_one(args):
    mean_inp, sigma_x_inp, corr_x_inp, sigma_g_inp, corr_g_inp, drop_prob, dim1, seq_len, err_idx, runs = args
    # print(args)
    x_out_sum = 0
    x_out_sq_sum = 0
    ...
    layer_output = torch.nn.functional.dropout(input_tensor, p=drop_prob)
    ...
    pred_x_out_mean = mean_inp
    pred_x_out_var = (sigma_x_inp * sigma_x_inp + drop_prob*mean_inp*mean_inp)/(1-drop_prob)
    pred_x_out_cov = corr_x_inp * sigma_x_inp * sigma_x_inp
    # pred_g_out_mean = 0
    pred_g_out_var = sigma_g_inp * sigma_g_inp / (1 - drop_prob)
    pred_g_out_cov = sigma_g_inp * sigma_g_inp * corr_g_inp
