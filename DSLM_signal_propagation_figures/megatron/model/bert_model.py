...
class BertLMHead(MegatronModule):
    ...
    def __init__(self, mpu_vocab_size, hidden_size, init_method,
        ...
        if args.moment_control_sigma:
            moment_control_sigma_lm_head_linear = 1.0/math.sqrt(args.hidden_size) # 0.044
        else:
            moment_control_sigma_lm_head_linear = 0.02
        self.dense = get_linear_layer(hidden_size, hidden_size, init_method(moment_control_sigma_lm_head_linear))
        ...
        if args.openai_gelu:
            ...
            self.gelu = erf_gelu
        if args.moment_control_sigma:
            self.moment_control_sigma_output_embd = 1.0/math.sqrt(hidden_size*0.3)
        else:
            self.moment_control_sigma_output_embd = 1.0

    def forward(self, hidden_states, word_embeddings_weight):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.gelu(hidden_states)
        hidden_states = self.layernorm(hidden_states)
        hidden_states *= self.moment_control_sigma_output_embd
    ...
    def __init__(self,
        ...
        if self.post_process:
            ...
            if self.add_binary_head:
                if args.moment_control_sigma:
                    moment_control_sigma_binary_head = 1.0/math.sqrt(args.hidden_size) # 0.044
                else:
                    moment_control_sigma_binary_head = 0.02
                self.binary_head = get_linear_layer(args.hidden_size, 2,
                                                    init_method(moment_control_sigma_binary_head))
