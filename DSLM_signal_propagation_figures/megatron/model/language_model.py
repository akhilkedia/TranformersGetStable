    ...
    def __init__(self,
        ...
        if self.pre_process:
            if args.moment_control_sigma:
                moment_control_sigma_embedding = math.sqrt((1-args.hidden_dropout)/3) #0.548
            else:
                moment_control_sigma_embedding = 0.02
            self.embedding = Embedding(self.hidden_size,
                                       args.padded_vocab_size,
                                       args.max_position_embeddings,
                                       args.hidden_dropout,
                                       self.init_method(moment_control_sigma_embedding),
                                       self.num_tokentypes)
        ...
        if self.post_process:
            ...
            if self.add_pooler:
                if args.moment_control_sigma:
                    moment_control_sigma_pooler = 1/math.sqrt(args.hidden_size) #0.044
                else:
                    moment_control_sigma_pooler = 0.02
                self.pooler = Pooler(self.hidden_size, self.init_method(moment_control_sigma_pooler))
